...
| 終端畫面重建 | Terminal Screen Reconstruction | 將 raw ANSI terminal data 套用 cursor movement 與 redraw 後得到可掃描文字行 | `mermaid_tui_preview/src/terminal.ts:TerminalScreen` |
| 終端預覽連結 | Terminal Preview Link | 附著於 `mermaid` marker 或 Mermaid directive、可觸發 diagram preview 的 terminal link | `mermaid_tui_preview/src/store.ts:ResolvedTerminalLink` |
